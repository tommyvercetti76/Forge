...
| 7 | **Inference-only is not ML** — calling someone else's checkpoint is a step short of training. | Pilot LoRA recipe trained on Apple Silicon: 50-image Mithila corpus → `mflux-train` on `z-image-turbo` with auto-derived captions, 4-bit quantization, 30-min smoke pilot end-to-end. Full overnight recipe (~15 hrs) documented. | [LORA_TRAINING_RECIPE](docs/LORA_TRAINING_RECIPE.md) · [forge_madhubani_lora.py](bin/forge_madhubani_lora.py) |
